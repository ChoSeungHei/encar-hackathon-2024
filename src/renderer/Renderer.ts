...
export class Renderer extends RendererOrigin {
    ...
    async MakeModelData() {
        ...
        this.camera.position[0] = this.camera_origin[0] = 420;
    ...
    showDamaged(modelName: string) {
        const obj = this.models.get(modelName);
        if(obj && typeof obj === 'object') {
            obj.isHighlighted = !obj.isHighlighted;
        }
    }

    movePinpoint(isExterior: boolean, dx: number) {
        if (isExterior) this.lookOrigin();
        else this.lookInner();

        this.rotateModel(this.model, dx, 0);
    }
    ...
    calculateModelCenter(vertices: number[]): [number, number, number] {
        ...
        return [(xSum / vertexCount) + 100, ySum / vertexCount, zSum / vertexCount];
    }
